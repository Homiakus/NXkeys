...
# Preserve catalog intents that share a BUTTON ID with universal selection actions.
    ...
    for (const [index, filter] of CANONICAL_SELECTION_FILTERS.entries()) {
      ...
      upsertCommand(
        ...
        () => ({
          ...
          input_key: filter.path[1], path: [...filter.path], path_labels: ['Select', filter.name], aliases: [],
          search_aliases: searches, icon_hint: filter.iconHint, display_order: 9000 + index,
          command: { id: filter.id, name: filter.name }, action: 'set_selection_filter',
          selection_type: filter.selectionType, enabled: true, requires_selection: false,
          destructive: false, confirm_before_execute: false,
...
# Synchronize declarative state-machine keys with the shortened canonical paths.
state_path = ROOT / "config/nx2512-state-machines.json"
state = json.loads(state_path.read_text(encoding="utf-8-sig"))
commands = state.get("commands", {})
for old_key, new_key in {
    "MEEB": "MEB", "MEEC": "MEC", "SCGL2": "SCL", "AECR": "AER",
    "AXCR": "AXC", "CPOP": "CPP", "CXOD": "CXO",
}.items():
...
# Correct the C# runtime source marker used by the ergonomic validator.
ergonomic_path = "scripts/validate-ergonomic-map.mjs"
ergonomic = read(ergonomic_path).replace(
...
write(ergonomic_path, ergonomic)

(ROOT / "scripts/sitecustomize.py").unlink(missing_ok=True)
(ROOT / "scripts/fix-ergonomic-v7.py").unlink(missing_ok=True)
print("Ergonomic v7 integration fixups applied.")
