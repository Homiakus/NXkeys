...
5. `G*` зарезервирован для переходов между приложениями NX.
6. Неоднозначные и неразрешённые команды видимы, но отключены.
7. Sketch использует отдельную смысловую грамматику и допускает пути до пяти токенов.
...
Sketch не использует случайные короткие пути. Его язык строится как:

```text
действие → область → операция → вариант
```
...
|---|---|
| `C → G → …` | создать геометрию |
| `E → G → …` | редактировать геометрию |
| `T → G → …` | преобразовать геометрию |
| `C → K → …` | создать геометрическое ограничение |
| `A → D → …` | создать размер |
| `I → S → …` | проверить эскиз |
| `X → G → …` | удалить или разорвать геометрию |
| `M → S → …` | настройки и управление Sketch |
...
Терминальная команда не может одновременно быть префиксом другой команды. Поэтому варианты вынесены в отдельную prefix-free ветвь:

```text
C → G → V → тип объекта → вариант
```
